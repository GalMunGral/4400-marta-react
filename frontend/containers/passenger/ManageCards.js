...
class ManageCards extends Component {
  ...
  _addNewCard() {
    const cardInfo = {
      ...
      Number: this.newCardNumberInput.value
    };
    if (cardInfo.Number.match(/^[0-9]{16}$/)) {
      ...
      this.props.showErrorMessage(
        `Breezecard number format error! Time: ${new Date()}`, 1
      );
    }
  }
  ...
  _addValueToCard() {
    ...
    if (this.creditCardNumberInput.value.match(/^[0-9]{16}$/)) {
      if (cardInfo.valueToAdd.match(/^[0-9]+(\.[0-9]{2})?$/)) {
        if (
          parseFloat(cardInfo.valueToAdd) + parseFloat(cards[selected].Value)
            <= 1000
          ...
          this.props.showErrorMessage(
            `Card value maximum exceeded! Time: ${new Date()}`, 1
          );
        }
      } else {
        this.props.showErrorMessage(
          `Invalid number format! Time: ${new Date()}`, 1
        );
      }
    } else {
      this.props.showErrorMessage(
        `Invalid Credit card number! Time: ${new Date()}`, 1
      );
    }
  }
  ...
  _removeCard(i) {
    ...
    if (cards.length === 1) {
      this.props.showErrorMessage(
        `Passenger must have at least one card! Time: ${new Date()}`, 1
      );
    } else {
        ...
        <Panel>
          ...
          <Col md={12}>
            <Panel header="Add Value to Selected Card">
                ...
                <FormGroup>
                  ...
                  <Col md={8}>
                    <FormControl
                      ...
                      onChange={e => {
                        if (e.target.value < 0) this.addValueInput.value = 0;
                      }}
                    />
                  </Col>
                </FormGroup>
              ...
              <Col mdOffset={9} md={3}>
                <Button onClick={this._addValueToCard}>Add Value</Button>
              </Col>
            </Panel>
          </Col>
        </Panel>
