...
class StationDetail extends Component {
  ...
  componentDidMount() {
    if (this.props.station !== undefined) {
      ...
      this.openClosedCheckbox.checked = (
        station.ClosedStatus === "Open" ? true : false
      );
  ...
  render() {
    const station = this.props.station;
    if (!station) return <Redirect to="/stations"/>; 
    return (
      <div>
        <FatCol>
          <ErrorMessage/>
          ...
          <Panel>
            <Form horizontal className="container-fluid">
              <FormGroup>
                ...
                <Col sm={2}>
                  <h3 style={{ color: '#d9534f' }}>{station.StopID}</h3>
                </Col>
              </FormGroup>
              <FormGroup>
              <Col componentClass={ControlLabel} sm={3}>Fare</Col>
              <Col sm={6}>
                <FormControl
                  type="number"
                  ...
                  onChange={e => { 
                    if (e.target.value < 0) this.fareInput.value = 0;
                  }}
                />
              </Col>
              <Col sm={3}>
                <Button onClick={this._updateFare}>Update</Button>
              </Col>
              </FormGroup>
              <FormGroup>
                ...
                <Col sm={6}>
                  {!this.props.station.IsTrain ? (
                    ...
                    <HelpBlock>Not available for train stations</HelpBlock>
                  )}
                </Col>
              </FormGroup>
              <FormGroup>
                <Col mdOffset={2} md={10}>
                  <Checkbox
                    inputRef={ref => { this.openClosedCheckbox = ref; }}
                  >
                    Open Station
                  </Checkbox>
                  <HelpBlock>
