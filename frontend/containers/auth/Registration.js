  ...
  _register() {
    if (
      ...
      const userInfo = {
        ...
        cardNo: (
          this.useExistingCardRadio.checked
      ...
      if (this.passwordInput.value === this.passwordInput2.value) {
        if (this.emailInput.value.match(/^.+?@.+?\..+$/)) {
          if (
            ...
            || userInfo.cardNo.match(/^[0-9]{16}$/)
          ) {
            this.props.closeErrorMessage();
            this.props.register(userInfo);
          } else {
            this.props.showErrorMessage(
              `Incorrect Breezecard number! Time: ${ new Date()}`,
              1
            );
          }
        } else {
          this.props.showErrorMessage(
            `Invalid email address! Time: ${ new Date()}`,
            1
          );
        }
      } else {
        this.props.showErrorMessage(
          `Two passwords do not match! Time: ${ new Date()}`,
          1
        );
      }
    } else {
      this.props.showErrorMessage(
        `Please fill out all fields! Time: ${ new Date()}`,
        1
      );
    }
  }
      ...
      <div>
        ...
        <SlimCol>
          <Panel>
            <Form horizontal>
              <FormGroup>
                <Col componentClass={ControlLabel} sm={2} md={4}>Username</Col>
                <Col sm={10} md={7}>
                  <FormControl
              ...
              <FormGroup>
                <Col componentClass={ControlLabel} sm={2} md={4}>Email</Col>
                <Col sm={10} md={7}>
                  <FormControl
              ...
              <FormGroup>
                <Col componentClass={ControlLabel} sm={2} md={4}>Password</Col>
                <Col sm={10} md={7}>
                  <FormControl
                    type="password"
            ...
            <Panel header="Breeze Card">
              <Radio
                name="chooseCardType"
                inputRef={ref => { this.useExistingCardRadio = ref; }}
              >
              ...
              <Radio
                name="chooseCardType"
                inputRef={ ref => { this.createNewCardRadio = ref; }
              }>
