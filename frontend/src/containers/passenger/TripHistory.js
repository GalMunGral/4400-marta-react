...
class TripHistory extends Component {
  ...
  _updateFilter() {
    ...
    if (
      (filter.start === "" || checkDateFormat(filter.start))
      && (filter.end === "" || checkDateFormat(filter.end))
    ) {
      this.props.updateHistoryFilter(filter);
    } else {
      this.props.showErrorMessage(
        `Date/time format error! Time: ${new Date()}`, 1
      );
    }
  }

  _resetFilter() {
    this.props.updateHistoryFilter({ start: "", end: "" });
          ...
          <Panel>
            <Col md={8}>
              <Form horizontal className="container-fluid">
                <FormGroup>
                  <Col componentClass={ControlLabel} md={4}>Start Time</Col>
                  <Col md={8}>
                    <FormControl
                ...
                <FormGroup>
                  <Col componentClass={ControlLabel} md={4}>End Time</Col>
                  <Col md={8}>
                    <FormControl
            ...
            <Col md={4}>
              <Form horizontal className="container-fluid">
                <FormGroup>
                  <Button block onClick={this._updateFilter}>Update</Button>
