...
export function removeCard(cardInfo) {
  return (dispatch) => {
    ...
    return fetch(`//${BASE_URL}/api/passenger/remove-card`, {
      method: 'POST',
...
export function fetchMyCards(user, order) {
  return (dispatch) => {
    ...
    return fetch(
      `//${BASE_URL}/api/passenger/my-cards?user=${user}&attr=${attr}&asc=${asc}`
    )
      .then(res => res.json())
      .then((json) => {
        if (json.err) {
...
export function addValueToCard(cardInfo) {
  return (dispatch) => {
    ...
    fetch(`//${BASE_URL}/api/passenger/add-value`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify(cardInfo)
    })
...
export function addNewCard(cardInfo) {
  return (dispatch) => {
    ...
    fetch(`//${BASE_URL}/api/passenger/new-card`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify(cardInfo)
    })
